...
class ACIDTester {
  ...
  async stressTest(duration: number = 10000): Promise<TestResult> {
    ...
    return {
      ...
      message: `Stress test: ${results.operations} ops, ${results.errors} errors, ${results.successRate.toFixed(2)}% success rate`,
      duration: Date.now() - start
    };
  }
  ...
  private async increment(): Promise<void> {
    await axios.post(`${this.apiUrl}/increment`, {}, {
      headers: { 'X-Request-ID': uuidv4() }
    });
  }

  private async decrement(): Promise<void> {
    await axios.post(`${this.apiUrl}/decrement`, {}, {
      headers: { 'X-Request-ID': uuidv4() }
    });
  }
}
